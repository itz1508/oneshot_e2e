...
def requirement_files(provider: str | None = None) -> list[Path]:
    ...
    if selected in {"adk_gemma2", "google_adk_gemma2"}:
        files.append(ROOT / "requirements" / "adk.txt")
    if selected in {"featherless", "featherless_gemma4"}:
        files.append(ROOT / "requirements" / "featherless.txt")
    if os.getenv("ONESHOT_WORKSPACE_API", "").lower() == "true":
        files.append(ROOT / "requirements" / "workspace-api.txt")
    return files
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.profile in ("base", "all"):
        errors.extend(pins(ROOT / "requirements" / "base.txt", True))
    if args.profile in ("adk", "all"):
        errors.extend(pins(ROOT / "requirements" / "adk.txt", True))
    if args.profile in ("featherless", "all"):
        errors.extend(pins(ROOT / "requirements" / "featherless.txt", True))
    if args.profile in ("workspace", "all"):
        errors.extend(pins(ROOT / "requirements" / "workspace-api.txt", True))
