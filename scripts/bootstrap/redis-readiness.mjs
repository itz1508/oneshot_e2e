...
export async function ensureQueueReadiness(opts = {}) {
  const required =
    process.env.ONESHOT_QUEUE_REQUIRED === "true" || opts.required === true;
  const allowDocker =
    process.env.ONESHOT_BOOTSTRAP_ALLOW_DOCKER !== "false" &&
    opts.allowDocker !== false;
  ...
  log(
    `Probing Redis at ${host}:${port} (REDIS_URL=${configuredUrl ? "set" : "unset → default"}) ...`,
  );
  if (await probe(host, port)) {
    log(`Redis reachable at ${host}:${port}`);
    return {
      ok: true,
      source: configuredUrl ? "configured-url" : "default-local",
      redisUrl: url,
  ...
  if (!required) {
    log(
      "Redis not reachable and ONESHOT_QUEUE_REQUIRED is not set — runs execute inline (fully functional).",
    );
    return {
      ok: true,
      source: "inline-fallback",
      redisUrl: url,
      message: "Redis unavailable; inline fallback (queue optional)",
    };
  }
  ...
  if (process.platform !== "win32" && hasCommand("redis-server")) {
    log(
      "Redis required but unreachable; redis-server found — starting a local instance (documented) ...",
    );
    try {
      execSync(
        'redis-server --daemonize yes --appendonly yes --port 6379 --save ""',
        { stdio: "ignore" },
      );
    } catch (e) {
      log(
        `redis-server start attempt failed: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
    if (await probe("127.0.0.1", 6379, 3000)) {
      ...
      return {
        ok: true,
        source: "local-redis-server",
        redisUrl: "redis://127.0.0.1:6379",
        message: "Started local redis-server",
      };
    }
  ...
  if (allowDocker && hasCommand("docker")) {
    log(
      "Docker found and policy allows it — launching a local Redis container (documented; 127.0.0.1 only; not public) ...",
    );
    try {
      execSync("docker rm -f oneshot-redis", { stdio: "ignore" });
    } catch {
      /* none */
    }
    try {
      execSync(
        "docker run -d --name oneshot-redis -p 127.0.0.1:6379:6379 redis:7-alpine redis-server --appendonly yes",
        { stdio: "ignore" },
      );
    } catch (e) {
      log(
        `docker launch attempt failed: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
    for (let i = 0; i < 20; i++) {
      if (await probe("127.0.0.1", 6379, 1000)) {
        ...
        return {
          ...
          redisUrl: "redis://127.0.0.1:6379",
          message: "Launched local Redis container",
        };
      }
