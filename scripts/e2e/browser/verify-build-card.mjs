...
try {
  await startSession();
  await ev(`localStorage.setItem('oneshot.currentRunId', ${JSON.stringify(RUN_ID)});`);
  ...
  const mut = await ev(`fetch('/api/runs/${RUN_ID}/artifacts/file-mutations').then(r => r.json())`);
