...
test("HTTP/UI path reaches DONE through ADK Gemma provider adapter", async () => {
  const saved = {
  ...
  try {
    ...
    assert.equal(
      snap.events.find((e: any) => e.processor === "Done" && e.state === "COMPLETE")
        ?.result,
      "PASSED",
    );
    ...
    assert.equal(graph.attached_to, "Researcher");
    assert.equal(graph.authority, "projection-only");
    assert.equal(
      graph.nodes.find((n: any) => n.id === "cache")?.state,
      "COMPLETE",
    );
    assert.equal(
      graph.nodes.find((n: any) => n.id === "research-draft")?.state,
      "COMPLETE",
    );
  } finally {
