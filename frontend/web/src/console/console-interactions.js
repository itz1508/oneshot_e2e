...
export function createConsoleInteractions({ apiFetch, toast }) {
    ...
    preview.querySelector("[data-preview-close]").onclick = () =>
        preview.close();
    ...
    document.querySelector("#preview-copy").onclick = async () => {
        try {
            ...
            toast("Clipboard unavailable. Select the file contents to copy.");
        }
    };
    ...
    document.querySelectorAll("[data-status-tab]").forEach((button) => {
        button.onclick = () => {
            ...
            document
                .querySelectorAll("[data-status-panel]")
                .forEach((panel) => {
                    panel.hidden =
                        panel.dataset.statusPanel !== button.dataset.statusTab;
                });
        };
    });
    ...
    async function showFile(title, url) {
        ...
        try {
            ...
            if (!response.ok)
                throw new Error(`File unavailable (${response.status})`);
    ...
    function renderStatus(
        ...
        overview.innerHTML = states.size
            ? [...states.values()]
                  .map(
                      (event) =>
                          `<div class="status-stage" data-state="${esc(event.state)}"><span>${esc(stageLabel(event.processor))}</span><strong>${esc(event.result || event.state)}</strong></div>`,
                  )
                  .join("")
            : '<p class="empty-note">Start a run to see its progress.</p>';
        ...
        activity.innerHTML = events.length
            ? [...events]
                  ...
                  .map(
                      (event) =>
                          `<div class="status-event"><small>#${esc(event.sequence)} · ${esc(stageLabel(event.processor))} · ${esc(event.state)}</small><p>${esc(event.message || event.activity || event.result || "State updated")}</p></div>`,
                  )
                  .join("")
            : '<p class="empty-note">No runtime events yet.</p>';
        ...
        files.innerHTML =
            Object.keys(snapshot?.artifacts || {})
                .map(
                    (name) =>
                        `<button class="artifact-preview" data-artifact="${esc(name)}"><span>▤</span>${esc(name)}<small>Preview</small></button>`,
                )
                .join("") ||
            '<p class="empty-note">Artifacts appear as the run produces them.</p>';
        files.querySelectorAll("[data-artifact]").forEach((button) => {
            button.onclick = () =>
                showFile(
                    button.dataset.artifact,
                    `/api/runs/${encodeURIComponent(runId)}/artifacts/${encodeURIComponent(button.dataset.artifact)}`,
                );
        });

        document.querySelector("#status-run-label").textContent =
            snapshot?.test_result ||
            (events.filter((e) => e.state !== "Pending").at(-1)?.processor
                ? `${stageLabel(events.filter((e) => e.state !== "Pending").at(-1).processor)}`
                : "No active run");
    }
