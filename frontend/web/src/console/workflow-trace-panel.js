...
function entryHtml(entry) {
    const details = Object.entries(entry.details || {})
        ...
        .join("");
    return (
        `<details class="trace-entry" data-event-id="${escapeText(entry.eventId)}">` +
        ...
        (entry.artifactId
            ? `<dt>artifact_id</dt><dd>${escapeText(entry.artifactId)}</dd>`
            : "") +
        (entry.message
            ? `<dt>message</dt><dd>${escapeText(entry.message)}</dd>`
            : "") +
        details +
        `</dl></details>`
    );
