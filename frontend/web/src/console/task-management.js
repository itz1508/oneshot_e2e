...
export function planToGroups(plan) {
    ...
    for (const s of steps) {
        ...
        const owner =
            typeof s.responsibility === "string" && s.responsibility.trim()
                ? s.responsibility.trim()
                : "Plan steps";
...
export function agentForResponsibility(owner) {
    ...
    if (
        ["schema validation", "fixture validation", "goal validation"].includes(
            o,
        )
    )
        return "Triple Validation";
...
export function createTaskManagement({ apiFetch } = {}) {
    const _apiFetch =
        apiFetch ||
        ((url, opts) => fetch(url, { credentials: "same-origin", ...opts }));
    ...
    function agentComplete(agent) {
        ...
        const s =
            groupEl(agent)?.querySelector(":scope > summary")?.dataset.state;
        return s === "Completed" || s === "Completed";
    }
    ...
    function aggregateState(agent) {
        const states = stageRows(agent).map(
            (r) => r.dataset.state || "Pending",
        );
        if (states.some((s) => s === "Running")) return "Running";
        if (states.some((s) => s === "Failed")) return "Failed";
        if (
            states.length &&
            states.every((s) => s === "Completed" || s === "Completed")
        )
            return "Completed";
        return "Pending";
    }
    ...
    function taskItem(it) {
        ...
        check.innerHTML =
            '<svg width="12" height="12" viewBox="0 0 12 12" fill="none"><path d="M2.5 6L5 8.5L9.5 3.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
        ...
        if (it.stepId) {
            ...
            chip.title =
                "Step state (provided only by step-scoped runtime events)";
    ...
    return {
        ...
        async onSnapshot(snapshot, runId) {
            if (waitingReason) {
                this.renderPlanMessage(
                    `Waiting for ${waitingReason} decision. The runtime will continue after your authorization.`,
                );
                return;
            }
            const artifacts =
                snapshot?.artifacts && typeof snapshot.artifacts === "object"
                    ? snapshot.artifacts
                    : {};
            const name = artifacts.plan
                ? "plan"
                : artifacts["plan.gap"]
                  ? "plan.gap"
                  : artifacts["plan.reviewed"]
                    ? "plan.reviewed"
                    : artifacts["plan.researcher"]
                      ? "plan.researcher"
                      : null;
            if (!name || !runId) {
            ...
            try {
                ...
                if (!r.ok)
                    throw new Error(`Plan record read failed: ${r.status}`);
                const plan = JSON.parse(await r.text());
                this.renderPlan(plan, name);
            } catch (err) {
                this.renderPlanMessage(
                    `Plan record could not be read: ${err.message || err}`,
                );
            }
        },
        renderPlan(plan, sourceName) {
            ...
            for (const g of groups) {
                ...
                if (!agent) continue;
                const container = q(
                    `[data-agent-tasks="${CSS.escape(agent)}"]`,
                );
                if (container) {
        ...
        setWaiting(reason) {
            ...
            this.renderPlanMessage(
                `Waiting for ${reason} decision. The runtime will continue after your authorization.`,
            );
        },
        clearWaiting() {
            waitingReason = null;
