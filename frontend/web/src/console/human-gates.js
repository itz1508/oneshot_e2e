...
const list = (items, render) =>
    items?.length
        ...
        : '<p class="empty-note">Not provided by this run.</p>';
...
export function buildReviewHTML(review, collapsed = false) {
    ...
    const signals = [
        ...
        [
            "build package ready",
            review.status === "pending" ? "READY" : "AUTHORIZED",
        ],
    ];
...
export function hasVerifiedHash(snapshot) {
    ...
    return (
        proof?.equal === true &&
        typeof proof.created_hash === "string" &&
        proof.created_hash.length > 0 &&
...
export function collectEdits(element, bundle, notes) {
    const objective =
        element.querySelector("[data-objective]")?.value.trim() ??
        bundle.goal?.objective ??
        "";
    const requirements = [];
    ...
    element.querySelectorAll("[data-step-id]").forEach((input) => {
        ...
        if (textarea)
            steps.push({ id: input.value, description: textarea.value.trim() });
    });
    return { objective, requirements, steps, notes: [...notes] };
}

export function createHumanGates({
    apiFetch,
    toast,
    onChanged,
    onResearchAgain,
    ...
    function card(id) {
        ...
        if (!element) {
            ...
            document.querySelector("#chat").prepend(element);
        }
        return element;
    }
    function renderBuild() {
        ...
        element
            .querySelector("[data-gate-reopen]")
            ?.addEventListener("click", () => {
                collapsed = false;
                renderBuild();
            });
        element.querySelectorAll("[data-gate-action]").forEach((button) =>
            button.addEventListener("click", async () => {
                if (deciding) return;
                deciding = true;
                const runId = activeRun,
                    hash = build.hash,
                    action = button.dataset.gateAction;
                ...
                error.textContent =
                    action === "approve"
                        ? "Confirming build…"
                        : "Returning to the confirmed summary…";
                try {
                    ...
                    if (!response.ok)
                        throw new Error(
                            data.error ||
                                `Build review failed (${response.status}). Refresh the package and try again.`,
                        );
                    if (runId !== activeRun) return;
                    build = data;
                    collapsed = action === "return";
                    renderBuild();
                    await onChanged?.();
                } catch (failure) {
    ...
    async function refresh(runId, nextSnapshot = snapshot) {
        ...
        request = (async () => {
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    if (token === generation && !deciding) {
                        build = data;
                        renderBuild();
                    }
                } else if (response.status !== 404)
                    throw new Error(
                        `Build review unavailable (${response.status})`,
                    );
                if (snapshot?.artifacts?.research_bundle) {
                    const research = await apiFetch(
                        `/api/runs/${encodeURIComponent(runId)}/artifacts/research_bundle`,
                    );
                    if (!research.ok)
                        throw new Error(
                            `Research summary unavailable (${research.status})`,
                        );
                    ...
                    const reviewContract = reviewResponse.ok
                    ...
                    if (editable) {
                        const noteInput =
                            element.querySelector("[data-review-note]");
                        const addNote = () => {
                            const value = noteInput.value.trim();
                            if (!value) return;
                            researchNotes.push(value);
                            noteInput.value = "";
                    ...
                    element
                        .querySelector("[data-research-again]")
                        ?.addEventListener("click", () => {
                            if (!editable) return;
                            onResearchAgain?.();
                        });
                    ...
                    element
                        .querySelector("[data-accept-research]")
                        ?.addEventListener("click", async (event) => {
                            ...
                            try {
                                ...
                                if (!result.ok)
                                    throw new Error(
                                        data.error ||
                                            `Research acceptance failed (${result.status})`,
                                    );
                                if (activeRun !== runId) return;
                                event.target.textContent = "Research accepted";
                                await onChanged?.();
                            } catch (error) {
                ...
                if (token === generation) request = undefined;
            }
        })();
        return request;
    }
    function reset() {
        ...
        collapsed = false;
        researchNotes = [];
        document.querySelector("#build-review-card")?.remove();
        document.querySelector("#research-summary-card")?.remove();
    }
