...
export function createLiveActivity({ maxRows = LIVE_ACTION_CAP } = {}) {
    ...
    function upsertArtifact(a) {
        ...
        const operation =
            a.operation || (a.source === "snapshot" ? op.textContent : "");
        if (operation) {
    ...
    function restoreSplit() {
        ...
        const saved = parseFloat(
            localStorage.getItem("oneshot.liveSplit") || "",
        );
        if (pane && !Number.isNaN(saved))
            pane.style.flexBasis = `${clampSplit(saved)}%`;
    }
    ...
    function bindDivider() {
        ...
        divider.addEventListener("pointerdown", (e) => {
            splitDrag = {
                id: e.pointerId,
                y: e.clientY,
                start: pane.getBoundingClientRect().height,
            };
        ...
        divider.addEventListener("pointermove", (e) => {
            ...
            const pct =
                ((splitDrag.start + (e.clientY - splitDrag.y)) / total) * 100;
    ...
    return {
        ...
        onSnapshot(snapshot) {
            ...
            if (!rows.length) {
                if (!box.querySelector(".live-row.artifact")) {
                    box.innerHTML =
                        '<div class="live-empty">No artifacts reported yet.</div>';
                }
                return;
            }
        ...
        reset() {
            ...
            if (action)
                action.innerHTML =
                    '<div class="live-empty">Actions will appear when the run starts.</div>';
            if (files)
                files.innerHTML =
                    '<div class="live-empty">No artifacts reported yet.</div>';
        },
