...
export const STATE_NAMES = Object.keys(VISUAL_DEFAULTS.stateColors);
...
export function mergeVisual(stored) {
    ...
    if (stored.intensity === "LOW" || stored.intensity === "HIGH")
        base.intensity = stored.intensity;
    if (stored.stateColors && typeof stored.stateColors === "object") {
        for (const name of STATE_NAMES) {
            ...
            if (
                ...
                /^#[0-9a-fA-F]{6}$/.test(c.primary)
            )
                base.stateColors[name].primary = c.primary;
            if (
                typeof c.secondary === "string" &&
                /^#[0-9a-fA-F]{6}$/.test(c.secondary)
            )
                base.stateColors[name].secondary = c.secondary;
            if (c.ambientStrength !== undefined)
                base.stateColors[name].ambientStrength = clampStrength(
                    c.ambientStrength,
                );
        }
    }
...
export function loadVisual() {
    try {
        return mergeVisual(
            JSON.parse(localStorage.getItem(VISUAL_KEY) || "null"),
        );
    } catch {
...
export function applyVisualSettings(v, app) {
    ...
    for (const name of STATE_NAMES) {
        ...
        app.style.setProperty(
            `--state-${key}-strength`,
            String(c.ambientStrength),
        );
    }
    app.style.setProperty(
        "--ambient-strength",
        v.intensity === "HIGH" ? "1.8" : "1",
    );
    app.style.setProperty(
        "--glow-strength",
        v.glow ? (v.intensity === "HIGH" ? "1.8" : "1") : "0",
    );
}
...
class ParticleSystem {
    ...
    draw() {
        ...
        for (const p of this.particles) {
            ...
            this.ctx.shadowColor = color.replace(
                "ALPHA",
                String(currentAlpha * 0.8),
            );
            this.ctx.fill();
        }
    }
