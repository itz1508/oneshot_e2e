...
const FIXTURE_DIR = 'fixtures/console/';
const read = p => fs.readFileSync(new URL(p, import.meta.url), 'utf8');
const exists = p => fs.existsSync(new URL(p, import.meta.url));
const h=read(FIXTURE_DIR+'index.html');
const c=read(FIXTURE_DIR+'styles.css');
const SRC_MODULES=['app.js','visual-settings.js','runtime-view-state.js','live-activity.js','task-management.js'];
const j=SRC_MODULES.map(f=>read(CONSOLE_DIR+f)).join('\n');

test('frontend modules exist and compose',()=>{
  for(const f of SRC_MODULES) assert.ok(exists(CONSOLE_DIR+f),`missing src/console/${f}`);
  const app=read(CONSOLE_DIR+'app.js');
