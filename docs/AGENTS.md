...
Role means IAM identity. Agents execute workflow behavior; skills provide reusable capabilities; tools perform operations; workflow controls order. Role operating instructions remain in `backend/role/*/SKILL.md`; they are agent instructions, not IAM definitions. Follow [the architecture rule](../.agents/rules/oneshot-skill-architecture.md). Reusable skills use `backend/skills/*`.
