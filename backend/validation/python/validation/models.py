...
NonEmptyUniqueStrList = Annotated[
    list[NonEmptyStr], Field(min_length=1), AfterValidator(_unique)
]
...
class RootCause(StrictModel):
    ...
    required_correction: NonEmptyStr
    recheck_target: NonEmptyStr
...
class Prompt(StrictModel):
    ...
    research_direction: NonEmptyUniqueStrList


class SuccessDefinition(StrictModel):
    success_criteria_ids: NonEmptyUniqueStrList
    success_meaning: NonEmptyStr
    evidence_ids: UniqueStrList


class Researcher(StrictModel):
    researcher_id: NonEmptyStr
    prompt_id: NonEmptyStr
    plan_id: NonEmptyStr
...
class PlanStep(StrictModel):
    ...
    depends_on: UniqueStrList
    requirement_refs: UniqueStrList
    goal_refs: UniqueStrList
    fixture_refs: UniqueStrList
    schema_refs: UniqueStrList


class RevisionEvidence(StrictModel):
    revision: Annotated[int, Field(ge=1)]
    affected_area: NonEmptyStr
    reason: NonEmptyStr
    audit_finding_id: NonEmptyStr
...
class Plan(StrictModel):
    ...
    revision: Annotated[int, Field(ge=1)]
    revision_evidence: list[RevisionEvidence]
...
class PlanAssertion(StrictModel):
...
class Fixture(StrictModel):
    fixture_id: NonEmptyStr
    researcher_id: NonEmptyStr
    plan_assertions: Annotated[list[PlanAssertion], Field(min_length=1)]


class SchemaValidationDefinition(StrictModel):
    plan_id: NonEmptyStr
    schema_id: NonEmptyStr


class FixtureValidationDefinition(StrictModel):
    plan_id: NonEmptyStr
    fixture_id: NonEmptyStr
    assertion_ids: UniqueStrList


class GoalValidationDefinition(StrictModel):
    plan_id: NonEmptyStr
    goal_id: NonEmptyStr
    criterion_ids: UniqueStrList
...
class ValidationDefinition(StrictModel):
    ...
    researcher_id: NonEmptyStr
    plan_id: NonEmptyStr
    schema_validation: SchemaValidationDefinition
    fixture_validation: FixtureValidationDefinition
    goal_validation: GoalValidationDefinition
...
class AuditFinding(StrictModel):
    ...
    area: NonEmptyStr
    finding: NonEmptyStr
    affected_plan_refs: UniqueStrList
    evidence_ids: UniqueStrList
    required_refinement: NonEmptyStr
...
class Audit(StrictModel):
    ...
    researcher_id: NonEmptyStr
    plan_id: NonEmptyStr
    reviewed_areas: UniqueStrList
    findings: list[AuditFinding]


class ResolvedGap(StrictModel):
    gap_id: NonEmptyStr
    affected_branch: NonEmptyStr
    issue: NonEmptyStr
    evidence_ids: UniqueStrList
    required_correction: NonEmptyStr
    expected_resolved_state: NonEmptyStr
    resolution_evidence: NonEmptyStr


class GapAnalysis(StrictModel):
    plan_id: NonEmptyStr
    result: Literal["Passed", "Failed"]
    resolved_gaps: list[ResolvedGap]
    gap_0: bool
    issue_type: Literal["Root Cause", "Missing"] | None = None
    root_cause: RootCause | None = None

    @model_validator(mode="after")
    def coherent(self):
        if self.result == "Passed" and (
            not self.gap_0 or self.root_cause is not None or self.issue_type is not None
        ):
            raise ValueError(
                "Passed Gap Analysis requires gap_0=true and no issue fields"
            )
        if self.result == "Failed" and (
            self.gap_0 or self.root_cause is None or self.issue_type is None
        ):
            raise ValueError(
                "Failed Gap Analysis requires gap_0=false and issue evidence"
            )
        return self
...
class Evaluation(StrictModel):
    plan_id: NonEmptyStr
    result: Literal["Passed", "Failed"]
    evidence: Annotated[list[EvaluationEvidence], Field(min_length=1)]
    issue_type: Literal["Root Cause", "Missing"] | None = None
    root_cause: RootCause | None = None
    ...
    def coherent(self):
        if self.result == "Passed" and (
            self.root_cause is not None or self.issue_type is not None
        ):
            raise ValueError("Passed Evaluation has no issue fields")
        if self.result == "Failed" and (
            self.root_cause is None or self.issue_type is None
        ):
            raise ValueError("Failed Evaluation requires issue evidence")
        return self
...
class FixtureValidationResult(StrictModel):
    plan_id: NonEmptyStr
    fixture_id: NonEmptyStr
...
class CriterionResult(StrictModel):
    criterion_id: NonEmptyStr
    mapped_plan_refs: UniqueStrList
    satisfied: bool


class GoalValidationResult(StrictModel):
    plan_id: NonEmptyStr
    goal_id: NonEmptyStr
...
class TripleValidation(StrictModel):
    plan_id: NonEmptyStr
    validation_id: NonEmptyStr
    schema_validation: SchemaValidationResult
    fixture_validation: FixtureValidationResult
    goal_validation: GoalValidationResult
    all_valid: bool


class ConfirmedCore(StrictModel):
    ...
    triple_validation: TripleValidation


class ConfirmedPackage(StrictModel):
    confirmed: Literal[True]
    core: ConfirmedCore


class HashProof(StrictModel):
    canonicalization_id: Literal["oneshot-jcs-rfc8785-v1"]
    algorithm: Literal["sha256"]
    created_hash: Sha256Str
    recomputed_hash: Sha256Str
    equal: bool


class GraphNode(StrictModel):
    id: NonEmptyStr
    kind: Literal["source", "processor", "validator", "gate", "proof", "terminal"]
...
class WorkflowAgent(StrictModel):
    ...
    @model_validator(mode="after")
    def coherent(self):
        if self.type == "SequentialAgent" and (
            self.exit is not None or self.join is not None
        ):
            raise ValueError("SequentialAgent cannot define exit or join")
        if self.type == "LoopAgent" and (self.exit is None or self.join is not None):
            raise ValueError("LoopAgent requires exit and cannot define join")
        if self.type == "ParallelAgent" and (
            self.join is None or self.exit is not None
        ):
            raise ValueError("ParallelAgent requires join and cannot define exit")
        return self


class WorkflowGraph(StrictModel):
    workflow_id: Literal["oneshot-canonical-workflow"]
    version: NonEmptyStr
    nodes: list[GraphNode]
    edges: list[GraphEdge]
    artifact_ownership: list[ArtifactOwnership]
    parallel_groups: list[ParallelGroup]
    workflow_agents: Annotated[list[WorkflowAgent], Field(min_length=1)]


class ContractRegistryEntry(StrictModel):
    contract_id: NonEmptyStr
    version: NonEmptyStr
    artifact_type: NonEmptyStr
    schema_path: NonEmptyStr
    schema_digest: Sha256Str
    producer: NonEmptyStr
    consumers: UniqueStrList


class ContractRegistry(StrictModel):
    registry_id: Literal["oneshot-contract-registry"]
