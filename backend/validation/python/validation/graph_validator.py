...
def validate_graph(graph: dict) -> list[str]:
    ...
    actual = {(edge["from"], edge["to"], edge["artifact"]) for edge in graph["edges"]}
    ...
    workflow_agents = {agent["id"]: agent for agent in graph.get("workflow_agents", [])}
    ...
    owners = {item["artifact"]: item["owner"] for item in graph["artifact_ownership"]}
    expected_owners = {
        "plan_id": "Researcher",
