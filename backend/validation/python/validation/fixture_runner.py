...
def run_assertion(assertion: dict, plan: dict, schema_store=None, graph=None):
    ...
    if op == "exists":
        ...
        if graph is None:
            ...
            ok = any(
                e.get("from") == exp.get("from")
                and e.get("to") == exp.get("to")
                and (
                    exp.get("artifact") is None
                    or e.get("artifact") == exp.get("artifact")
                )
                for e in edges
            )
            actual = ok
...
def run_fixture(
    ...
    by_id = {a["assertion_id"]: a for a in fixture["plan_assertions"]}
    results = []
    for aid in wanted:
        if aid not in by_id:
            results.append(
            ...
            results.append(run_assertion(by_id[aid], plan, schema_store, graph))
