...
def evaluate_plan(plan:dict,goal:dict,researcher:dict|None=None,fixture:dict|None=None,schema_artifact:dict|None=None,validation:dict|None=None)->dict:
    ...
    if failed:return {'plan_id':plan['plan_id'],'result':'Failed','issue_type':'Root Cause','evidence':evidence,'root_cause':{'issue':'Evaluation evidence incomplete','expected':'All canonical Evaluation areas complete','actual':', '.join(failed),'evidence_ids':evidence_ids,'required_correction':'Correct failed Evaluation areas','recheck_target':plan['plan_id']}}
