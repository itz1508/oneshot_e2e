...
PRODUCER_CONSUMERS = {
    ...
    "evaluation": (
        "Evaluation",
        ["SchemaValidation", "FixtureValidation", "GoalValidation"],
    ),
...
def build_registry(schema_dir: str | Path):
    ...
    for p in sorted(sd.glob("*.schema.json")):
        ...
        s = json.loads(b)
        name = p.name[:-12]
        prod, cons = PRODUCER_CONSUMERS[name]
        contracts.append(
            {
                "contract_id": s["$id"],
                "version": "2",
                "artifact_type": name,
                "schema_path": "schema/" + p.name,
                "schema_digest": hashlib.sha256(b).hexdigest(),
                "producer": prod,
                "consumers": cons,
            }
        )
    return {
        "registry_id": "oneshot-contract-registry",
        "registry_version": "2",
        "contracts": contracts,
    }
