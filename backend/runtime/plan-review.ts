...
export class PlanReviewService {
  ...
  async get(runId: string): Promise<PlanReview | undefined> {
    return loadOptionalArtifact<PlanReview>(this.store, runId, "review");
  }
  async open(
    runId: string,
    research: ResearchBundle,
  ...
  async decide(
    ...
    try {
      ...
      if (!review)
        throw new PlanReviewError("No draft is awaiting review", 404);
      if (review.status !== "pending")
        throw new PlanReviewError("This review has already been decided");
  ...
  async wait(
    ...
    while (!isTerminal() && Date.now() < deadline) {
      ...
      if (review?.status === "cancelled")
        throw new Error("Plan review cancelled by the user");
      if (review?.status === "approved") return applyPlanReviewEdits(review);
...
function nonempty(value: unknown): string {
  if (typeof value !== "string" || !value.trim() || value.length > 20000)
    throw new PlanReviewError(
      "Review fields must contain between 1 and 20000 characters",
      400,
    );
  return value.trim();
}
...
function keyedEdits<T extends "statement" | "description">(
  ...
  if (!Array.isArray(items) || items.length !== originals.length)
    throw new PlanReviewError(
      "Keep the draft requirement and task identities",
      400,
    );
  return originals.map((original) => {
    ...
    if (matches.length !== 1)
      throw new PlanReviewError(
        "Review contains an unknown or duplicate identity",
        400,
      );
    return { id: original.id, [field]: nonempty(matches[0][field]) } as {
      id: string;
    } & Record<T, string>;
  });
}

export function validatePlanReviewEdits(
  value: unknown,
  review: PlanReview,
): PlanReviewEdits {
  if (!value || typeof value !== "object")
    throw new PlanReviewError("Review edits are required", 400);
  const edits = value as PlanReviewEdits;
  if (!Array.isArray(edits.notes) || edits.notes.length > 50)
    throw new PlanReviewError("At most 50 review notes are supported", 400);
  return {
    objective: nonempty(edits.objective),
    requirements: keyedEdits(
      edits.requirements,
      review.edits.requirements,
      "statement",
    ) as PlanReviewEdits["requirements"],
    steps: keyedEdits(
      edits.steps,
      review.edits.steps,
      "description",
    ) as PlanReviewEdits["steps"],
...
export function applyPlanReviewEdits(review: PlanReview): ResearchBundle {
  ...
  for (const requirement of research.plan.requirements) {
    requirement.statement = review.edits.requirements.find(
      (r) => r.id === requirement.requirement_id,
    )!.statement;
  }
  for (const step of research.plan.steps) {
    step.description = review.edits.steps.find(
      (s) => s.id === step.step_id,
    )!.description;
  }
  research.prompt.context.push(
    ...review.edits.notes.map((statement, index) => ({
      context_id: `review:${review.run_id}:note:${index + 1}`,
      statement,
    })),
  );
  research.researcher.evidence.push({
    evidence_id: `review:${review.run_id}`,
    source: "user-plan-review",
    statement: JSON.stringify(review.edits),
    provenance: "user-confirmed-draft",
  });
  return research;
}
