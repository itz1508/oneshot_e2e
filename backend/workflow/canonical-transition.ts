import type {
  ...
  PipelineStage,
  StageOutcome,
} from "../pipeline/stage-outcome.js";
...
function failedFinalization(
  outcome: Extract<StageOutcome, { kind: "terminal" | "refine" }>,
): PipelineTransition {
  return {
    type: "next",
    stage: "finalize",
...
export function resolveTripleValidationTransition(
  ...
  if (outcome.kind === "refine") {
    if (iteration >= MAX_REFINEMENT_ITERATIONS)
      return failedFinalization(outcome);
...
export function resolveTransition(
  ...
  if (outcome.kind === "terminal" && stage !== "finalize")
    return failedFinalization(outcome);

  switch (stage) {
    case "researcher":
      return { type: "wait-human" };
    case "planner":
      return { type: "next", stage: "refactor", iteration: 0 };
    case "refactor":
      return { type: "next", stage: "gap-analysis", iteration };
    case "gap-analysis":
      return outcome.kind === "refine"
    ...
    case "triple-validation":
      return resolveTripleValidationTransition(outcome, iteration);
    case "confirmation":
      return { type: "next", stage: "hash", iteration: 0 };
    case "hash":
      return { type: "wait-build" };
    case "build":
      return { type: "next", stage: "finalize", iteration: 0 };
    case "finalize":
