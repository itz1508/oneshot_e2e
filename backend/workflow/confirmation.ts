...
export class ConfirmationWorkflow {
  ...
  async run(
    ...
    if (!triple.all_valid)
      throw new Error("Triple Validation is not all VALID");
    const confirmed: ConfirmedPackage = {
      ...
      core: {
        ...
        gap_analysis: gap,
        evaluation,
        triple_validation: triple,
      },
    };
    await this.contracts.validate(
      "urn:oneshot:schema:confirmed-package:2",
      confirmed,
    );
    await this.contracts.validateReferences(confirmed.core);
    return confirmed;
  }
}
