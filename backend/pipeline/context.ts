...
export async function loadPrompt(
  ...
  if (!promptId) {
    throw new Error(
      `No prompt artifact found for run ${ctx.runId}`,
    );
  }
...
export async function loadGapAnalysis(
  ...
  return ctx.store.load<GapAnalysis>(
    ctx.runId,
    "gap_analysis",
  );
}
...
export async function loadEvaluation(
  ...
  return ctx.store.load<Evaluation>(
    ctx.runId,
    "evaluation",
  );
}
...
export async function loadTripleValidation(
  ...
  return ctx.store.load<TripleValidation>(
    ctx.runId,
    "triple_validation",
  );
}
...
export async function loadConfirmedPackage(
  ...
  return ctx.store.load<ConfirmedPackage>(
    ctx.runId,
    "confirmed",
  );
}

export async function loadHashProof(
  ctx: PipelineContext,
): Promise<HashProof> {
  return ctx.store.load<HashProof>(
    ctx.runId,
    "hash_proof",
  );
}
...
export async function saveArtifact<T>(
  ...
  const path = await ctx.store.save(
    ctx.runId,
    name,
    value,
  );
