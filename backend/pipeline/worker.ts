...
import { advance, refine, terminal } from "./stage-outcome.js";
import type {
  PipelineIssue,
  PipelineStage,
  StageOutcome,
...
export function createPipelineWorker(
  ...
  const { runs, store, services, redis, history, concurrency = 1 } = input;
  ...
  const worker = new Worker<StageJobData, unknown, PipelineStage>(
    PIPELINE_QUEUE,
    ...
    async (job: PipelineJob) => {
      ...
      if (
        ...
        !Number.isInteger(job.data.iteration) ||
        job.data.iteration < 0
      ) {
      ...
      const progress = async (value: StageProgress): Promise<void> => {
        await job.updateProgress({
          ...value,
          runId,
          iteration: stageIteration(stage, requestedIteration),
        });
      };
      ...
      const identity: StageIdentity = {
        ...
        iteration:
          stage === "researcher"
            ? requestedIteration
            : stageIteration(stage, requestedIteration),
      };

      const execute = async (): Promise<StageOutcome> => {
        /*
          ...
          return refine(
            { faultInjected: true },
            {
              ...
              evidence: {
                ...
                evidence_ids: [],
                required_correction: "Run the next refinement iteration",
        ...
        switch (stage) {
          ...
          case "triple-validation": {
            await runTripleValidationStage(ctx, services, progress);
            ...
            return triple.all_valid
              ...
              : refine(
                  { all_valid: false },
                  {
                    issue_type: "Missing",
                    evidence: {
                      issue: "Schema, fixture, or goal validation failed",
                      expected:
              ...
              : terminal(
                  { equal: false },
                  {
                    ...
                    evidence: {
                      ...
                      actual: "Hash proof reported a canonical mismatch",
                      evidence_ids: [proof.created_hash],
                      required_correction:
          ...
          case "build": {
            ...
            if (!firstExecution) {
              try {
                const recovered = await store.load<{ result: string }>(
                  runId,
                  "build_result",
                );
                return recovered.result === "Passed"
                  ...
                  : terminal(
                      { result: recovered.result, recovered: true },
                      {
                        issue_type: "Root Cause",
                        evidence: {
                          ...
                          evidence_ids: [],
                          required_correction:
                            "Correct the sandbox build failure",
                ...
                return terminal(
                  { recovered: false },
                  {
                    ...
                    evidence: {
                      ...
                      actual:
                        "Execution intent exists without committed evidence",
                      evidence_ids: [],
                      required_correction:
                        "Inspect the sandbox execution and start a new run",
            ...
            return buildResult.result === "Passed"
          ...
          case "finalize": {
             ...
             */
            const intent = await checkpoints.loadFinalizationIntent(runId);

            await runFinalizeStage(ctx, services, progress);

            if (intent && intent.test_result === "Failed") {
              return terminal({ finalized: true }, intent.issue);
            }
            const proof = await loadHashProof(ctx);
            const buildResult = await store.load<{
              result: string;
            ...
            const sandboxHash =
              buildResult.hash_sandbox ?? buildResult.evidence?.hash_sandbox;
            if (
              buildResult.result !== "Passed" ||
              sandboxHash !== proof.created_hash
            ) {
              return terminal(
                { finalized: false },
                {
                  ...
                  evidence: {
                    ...
                    evidence_ids: [],
                    required_correction:
                      "Preserve and compare Builder sandbox hash evidence",
      ...
      const { skipped, outcome } = await runStage({
        identity,
        checkpoints,
        history: history ?? {
          append: async () => "",
        ...
        faults: {
          ...
          applyAfterCheckpoint: (faultRunId, faultStage, faultIteration) =>
        ...
        markCompleted: () =>
          idempotency.markCompleted(runId, stage, identity.iteration),
        job,
      });
       ...
       */
      const transition = resolveTransition(stage, outcome, identity.iteration);

      await applyTransition(identity, transition, transitionServices);
  ...
  worker.on("completed", (job) => {
    console.log(`[OneShot] ${job.name} completed: ${job.id}`);
  });

  worker.on("failed", (job, error) => {
    console.error(`[OneShot] ${job?.name ?? "unknown"} failed:`, error);
  });

  worker.on("error", (error) => {
    console.error("[OneShot] BullMQ worker error:", error);
  });
