...
class TestE2E(unittest.TestCase):
 def setUp(self):
  self.b=json.loads((ROOT/'app/fixtures/e2e/complete-success.json').read_text()); self.s=SchemaStore(ROOT/'backend/schema'); self.g=json.loads((ROOT/'backend/workflow/graph.json').read_text())
 def build_confirmed(self):
  ...
  pkg={'confirmed':True,'core':core}; self.s.assert_valid('urn:oneshot:schema:confirmed-package:2',pkg); self.assertEqual([],validate_references(core)); return pkg
 def test_complete_success_hash_equality(self):
  pkg=self.build_confirmed(); h=create_hash(pkg['core']); proof=verify_hash(pkg['core'],h); self.assertTrue(proof['equal']); self.s.assert_valid('urn:oneshot:schema:hash-proof:2',proof)
 def test_schema_not_valid(self):
  ...
  t=run_triple(self.b['plan'],self.b['validation'],bad,self.b['fixture'],self.b['goal'],self.s,self.g); self.assertEqual('Failed',t['schema_validation']['result']); self.assertFalse(t['all_valid'])
 def test_goal_not_valid(self):
  bad=copy.deepcopy(self.b['plan']); bad['steps'][0]['goal_refs']=[]
  t=run_triple(bad,self.b['validation'],self.b['schema_artifact'],self.b['fixture'],self.b['goal'],self.s,self.g); self.assertEqual('Failed',t['goal_validation']['result']); self.assertFalse(t['all_valid'])
