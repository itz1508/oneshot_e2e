...
class TestRuntimeParityExtended(unittest.TestCase):
    ...
    def test_graph_parity(self):
        p=prove_case(self.s,'urn:oneshot:schema:workflow-graph:2',self.g)
    ...
    def test_registry_parity(self):
        ...
        p=prove_case(self.s,'urn:oneshot:schema:contract-registry:2',r)
        self.assertTrue(p['parity']); self.assertTrue(p['schema_accepts'])
    ...
    def test_hash_pattern_parity(self):
        ...
        for value,expected in [(good,True),(bad,False)]:
            p=prove_case(self.s,'urn:oneshot:schema:hash-proof:2',value)
    ...
    def test_confirmed_and_result_contract_parity(self):
        ...
        items=[
            ('urn:oneshot:schema:schema-validation:2',t['schema_validation']),
            ('urn:oneshot:schema:fixture-validation:2',t['fixture_validation']),
            ('urn:oneshot:schema:goal-validation:2',t['goal_validation']),
            ('urn:oneshot:schema:triple-validation:2',t),
        ]
        ...
        pkg={'confirmed':True,'core':core}; items.append(('urn:oneshot:schema:confirmed-package:2',pkg))
        h=create_hash(core); items.append(('urn:oneshot:schema:hash-proof:2',verify_hash(core,h)))
    ...
    def test_empty_identifier_parity(self):
        ...
        p=prove_case(self.s,'urn:oneshot:schema:researcher:2',bad)
