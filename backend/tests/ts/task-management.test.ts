...
test("Task event stream is append-only, replayable, traced, and workflow-projectable", async () => {
  ...
  bus.emit("r1", "Integration:cache", "Running", { scope: "SUPPORT", message: "lookup" });
  bus.emit("r1", "Integration:cache", "Completed", { scope: "SUPPORT", message: "hit" });
  ...
  const graph = projectWorkflowGraph(reloaded);
  assert.equal(
    graph.nodes.find((n: { id: string }) => n.id === "Researcher")?.state,
    "Completed",
  );
  assert.equal(
    graph.nodes.find((n: { id: string }) => n.id === "Integration:active")?.state,
    "Completed",
  );
