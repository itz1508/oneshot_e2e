...
export interface StructuredResearchDraft {
  /** Optional model-authored text/deliverable, carried in confirmed plan when supplied. */
  deliverable?: string;
  summary: string;
  requirements: string[];
...
export interface BuildResearchBundleInput {
  projectRoot: string;
  prompt: Prompt;
  runId: string;
...
/**
 * Construct the canonical Researcher-owned bundle from a structured research draft.
 * Model output types stop here; downstream OneShot workflow consumes canonical ResearchBundle only.
 */
export async function buildResearchBundle(
  input: BuildResearchBundleInput,
  ...
  const {
    ...
    modelSource,
    modelProvenance,
    incompleteIssue = "RESEARCH_DRAFT_INCOMPLETE",
    incompleteCorrection = "Ensure requirements, plan_steps, and success_criteria are all non-empty",
  } = input;
