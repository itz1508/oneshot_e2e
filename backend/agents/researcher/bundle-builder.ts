...
/**
 * Researcher-owned intermediate representation of the model response.
 * Provider transports know nothing about this type.
 */
...
export async function researchDraftToBundle(
  input: BundleInput,
  ...
  if (
    ...
    throw new WorkflowRootCauseError({
      issue: "Researcher draft incomplete",
      expected: "requirements, plan_steps, and success_criteria are non-empty",
      actual: "Researcher structured draft omitted required sections",
      evidence_ids: [],
      required_correction:
