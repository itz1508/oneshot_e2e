...
def build_tool_registry(root:str|Path)->ToolRegistry:
    root=Path(root);store=SchemaStore(root/'backend/schema');reg=ToolRegistry()
    def validate_schema_tool(i):
        ...
        return {'valid':True,'count':validate_all_schema_documents(root/'backend/schema')}
    ...
    def run_fixture_tool(i):
        graph=i.get('graph') or json.loads((root/'backend/workflow/graph.json').read_text());results,ok=run_fixture(i['fixture'],i['plan'],store,graph,i.get('assertion_ids'));return {'results':results,'valid':ok}
