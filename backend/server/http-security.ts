...
export class HttpSecurity {
  ...
  private token=process.env.ONESHOT_API_TOKEN||"";
  ...
  allowed(req:IncomingMessage,res:ServerResponse){
    ...
    if(req.url?.startsWith("/api/")){
      ...
      if(this.token&&req.url!=="/api/health"&&req.headers.authorization!==`Bearer ${this.token}`){res.writeHead(401,{"content-type":"application/json; charset=utf-8"});res.end(JSON.stringify({error:"unauthorized"}));return false;}
    }
    return true;
  }
}
