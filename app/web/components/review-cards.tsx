...
import type { BuildReview, Bundle, Edits, Mutation, ResearchReview, Run } from '../lib/contracts';
...
export function ResearchCard({
  ...
  const [edits, setEdits] = useState<Edits>(() => review?.edits || editsFromBundle(bundle));
  ...
  const removeNote = (index: number) => {
    setEdits({ ...edits, notes: edits.notes.filter((_, i) => i !== index) });
  };
  ...
  return (
    <section className="card" id="research-summary-card" data-testid="research-summary-card">
      <header className="card-topbar">
        <div>
          <span className="card-tagline">Human Review · 01</span>
          <h2 className="card-title">Research Review</h2>
        </div>
        <span className={`card-status-pill ${waiting ? 'awaiting' : 'accepted'}`}>
          {waiting ? 'Awaiting acceptance' : 'Research baseline'}
        </span>
      </header>

      <p className="card-lead">
        Review the research baseline for this target. You may edit the objective, requirements, and plan steps, request Research Again, or accept to proceed to planning.
      </p>

      {waiting && (
        <div className="review-form-block">
          <div className="review-field-group">
            <label className="field-label">Objective (Editable)</label>
            <textarea
          ...
          <div className="review-field-group">
            <label className="field-label">Requirements ({requirements.length})</label>
            {requirements.length ? (
                  ...
                  <li key={r.id || `req-${i}`}>
                    <textarea
                      ...
                      onChange={e =>
                        setEdits({
                          ...edits,
                          requirements: edits.requirements.map((v, n) =>
                            n === i ? { ...v, statement: e.target.value } : v
                          ),
                        })
                      }
          ...
          <div className="review-field-group">
            <label className="field-label">Plan Steps ({steps.length})</label>
            {steps.length ? (
              <ul className="review-list-editable">
                {steps.map((s, i) => (
                  <li key={s.id || `step-${i}`}>
              ...
              <p className="text-dim">No plan steps generated yet.</p>
            )}
          </div>
          ...
          <div className="review-field-group">
            ...
            <div className="note-input-row">
              ...
              />
              <button type="button" className="btn btn-secondary" onClick={addNote}>
                Add Note
              </button>
            </div>
      ...
      {!waiting && (
        <div className="review-readonly-summary">
          <div className="review-field-group">
            ...
            <p className="readonly-text">{edits.objective || '—'}</p>
          </div>
          <div className="review-field-group">
      ...
      <details className="card-collapsible">
        ...
        <div className="card-collapsible-body">
          {bundle.researcher?.evidence?.map((e, i) => (
            <p key={i}>
              ...
              <small className="block text-dim">
                {e.source} {e.provenance ? `· ${e.provenance}` : ''}
              </small>
            </p>
          ))}
          ...
          {bundle.fixture?.plan_assertions && Array.isArray(bundle.fixture.plan_assertions) && (
            <pre className="fixture-json">
              {JSON.stringify(bundle.fixture.plan_assertions, null, 2)}
            </pre>
          )}
        </div>
      </details>
      ...
      {waiting && (
        <footer className="card-footer-actions">
          <button type="button" className="btn btn-secondary" disabled={busy} onClick={onAgain} data-research-again="true">
            Research Again
          </button>
          <button
            type="button"
            ...
            disabled={
              ...
              edits.requirements.some(r => !r.statement.trim()) ||
...
export function BuildCard({
  ...
  return (
    <section className="card" id="build-review-card" data-testid="build-review-card">
      <header className="card-topbar">
        <div>
          <span className="card-tagline">Human Review · 02</span>
          <h2 className="card-title">
            {review.status === 'approved' ? 'Build Authorized' : 'Build Ready'}
          </h2>
        </div>
        <span
        ...
        >
          {review.status === 'approved' ? 'Authorized' : 'Awaiting authorization'}
        </span>
      </header>

      <p className="card-lead">
        The confirmed package passed deterministic Triple Validation. Builder is waiting in the isolated sandbox for your explicit authorization.
      </p>

      <div className="validation-signals-grid">
        <div className="sig-box">
          <span className="sig-label">Schema Validation</span>
          <span className="sig-val">{review.validation?.schema || 'VALID'}</span>
        </div>
        <div className="sig-box">
          <span className="sig-label">Fixture Validation</span>
          <span className="sig-val">{review.validation?.fixture || 'VALID'}</span>
        </div>
        <div className="sig-box">
          <span className="sig-label">Goal Validation</span>
          <span className="sig-val">{review.validation?.goal || 'VALID'}</span>
        </div>
      </div>
      ...
      <details className="card-collapsible" open={!collapsed}>
        <summary>Confirmed Package Steps ({review.steps?.length || 0})</summary>
        <div className="card-collapsible-body">
          <ul className="timeline-list">
            {review.steps?.map(s => (
              <li key={s.step_id} className="timeline-item">
                <strong>{s.responsibility || 'Builder'}:</strong> {s.description}
              </li>
            ))}
          </ul>
        </div>
...
export function MutationsTable({ records }: { records: Mutation[] | null }) {
  if (!records || !records.length) {
    return <p className="text-dim">No workspace file mutations recorded.</p>;
  }
  ...
  return (
    <section className="card" id="final-result-card">
      <header className="card-topbar">
        ...
        <span
          className={`card-status-pill ${
            verified ? 'passed' : run.test_result === 'Failed' ? 'failed' : 'awaiting'
          }`}
        >
          {verified
            ? 'VERIFIED'
      ...
      <p className="card-lead">
        Job <code className="mono">{run.run_id}</code> has completed processing. Verified deterministic proof recorded below.
      </p>
      ...
      <div className="validation-signals-grid">
        ...
        <div className="sig-box">
          <span className="sig-label">Hash Equality (hash == hash_sandbox)</span>
          <span
            className="sig-val"
            style={{
              color: hashEqual ? 'var(--accent-emerald)' : 'var(--accent-rose)',
            }}
          >
        ...
        <div className="sig-box">
          ...
          <span className="sig-val">{run.current_processor || 'Done'}</span>
        </div>
      </div>
      ...
      {proof?.recomputed_hash && (
        <div className="hash-proof-row">
          ...
          <code className="mono hash-code">{proof.recomputed_hash}</code>
        </div>
      )}
