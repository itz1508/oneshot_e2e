...
import { clone } from "../../../../backend/core/clone.js";
...
function rewrite(value: unknown, map: Map<string, string>): unknown {
  ...
  if (value && typeof value === "object")
    return Object.fromEntries(
      Object.entries(value).map(([k, v]) => [k, rewrite(v, map)]),
    );
  return value;
}
export class FixtureResearchProvider implements ResearchProvider {
  ...
  async research(prompt: Prompt, runId: string): Promise<ResearchBundle> {
    ...
    const map = new Map<string, string>([
      ["researcher:seed", `researcher:${runId}`],
      ["plan:seed", `plan:${runId}`],
      ["schema:seed", `schema:${runId}`],
      ["fixture:seed", `fixture:${runId}`],
      ["goal:seed", `goal:${runId}`],
      ["validation:seed", `validation:${runId}`],
      ["req:seed", `req:${runId}`],
      ["criterion:seed", `criterion:${runId}`],
      ["assertion:seed", `assertion:${runId}`],
