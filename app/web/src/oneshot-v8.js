...
async function loadTargetInfo() {
    try {
        ...
        $("#target-value").textContent = data.root
...
async function loadConversation() {
    ...
    try {
        state.conversation = await apiJson(
...
async function startRun(reviewPlan = true) {
    const data = await apiJson(
        ...
        { method: "POST", body: JSON.stringify({ review_plan: reviewPlan }) },
    );
...
function renderConversation() {
    ...
    const turns = Array.isArray(state.conversation?.turns)
    ...
    for (const turn of turns) {
        ...
        const assistantText =
            turn.assistant_message || turn.message || turn.text || "";
        if (userText) {
            const group = document.createElement("div");
...
function listHtml(items, render) {
    if (!items?.length)
        return '<p class="empty-note">Not provided by this run.</p>';
...
async function ensureResearchCard() {
    ...
    try {
        const review = await apiJson(
            `/api/runs/${encodeURIComponent(state.runId)}/review`,
        );
        ...
        const requirements = Array.isArray(bundle.plan?.requirements)
            ? bundle.plan.requirements
            : [];
        const steps = Array.isArray(bundle.plan?.steps)
            ? bundle.plan.steps
            : [];
        const evidence = Array.isArray(bundle.researcher?.evidence)
      ...
      <footer><button class="btn secondary" data-research-again>Research Again</button><button class="btn primary" data-accept-research>Accept Research</button></footer>`;
        ...
        state.creatingResearchCard = false;
    }
}
...
function bindResearchCard() {
    ...
    if (againBtn && !againBtn.dataset.bound) {
        ...
        againBtn.addEventListener("click", () => {
            ...
            updatePromptMode();
            toast(
                "Research Again mode active. Type your follow-up instruction.",
            );
        });
    }
...
async function ensureBuildCard() {
    ...
    try {
        const review = await apiJson(
            `/api/runs/${encodeURIComponent(state.runId)}/build-review`,
        );
        if (!review) return;
        removeLoadingIndicator();
        const card = document.createElement("div");
...
function ensureSandboxCard() {
    if (!state.runId || !state.run || state.run.pipeline_status === "Done")
        return;
    if ($("#sandbox-card")) {
        updateSandboxCard();
        return;
    ...
    if (
        state.run.current_processor !== "Builder" &&
        state.run.pipeline_status !== "Running"
    )
        return;
...
function updateSandboxCard() {
    ...
    if (status)
        status.textContent = `${phaseLabel(state.run.current_processor || "Builder")} · ${state.run.pipeline_status || "Running"}`;
}
...
function renderCurrentJob() {
    ...
    const realTasks =
        Array.isArray(r.tasks) && r.tasks.length > 0 ? r.tasks : [];
    if (realTasks.length) {
        tasks.hidden = false;
        taskList.innerHTML = realTasks
            .map(
                (t) =>
                    `<li>${escapeHtml(t.description || t.name || t.id)}</li>`,
            )
            .join("");
    } else {
        tasks.hidden = true;
    ...
    $("#progress-bar").style.width =
        `${progress >= 0 ? ((progress + 1) / STAGE_ORDER.length) * 100 : 0}%`;
    const hook = $("#hook-state");
    const waiting =
        r.pipeline_status === "Waiting" ||
        /review|ready|hook|question/i.test(r.current_processor || "");
    hook.hidden = !waiting;
...
async function handleEvent(data) {
    try {
        ...
        state.run.current_processor =
            data.processor || state.run.current_processor;
        state.run.pipeline_status =
            data.execution_status || state.run.pipeline_status;
        state.run.test_result = data.test_result ?? state.run.test_result;
        renderCurrentJob();
        if (
            data.processor === "Researcher" &&
            data.execution_status === "Completed"
        ) {
            await refreshRunState();
            await ensureResearchCard();
        } else if (
            data.processor === "Confirmed" ||
            data.processor === "Hash Verification"
        ) {
            await refreshRunState();
...
function updatePromptMode() {
    ...
    if (state.intentKind === "research-again") {
        ...
        $("#composer-input").placeholder =
            "Tell OneShot what to research differently…";
    } else {
        mode.hidden = true;
        $("#composer-input").placeholder =
...
async function onSubmit(event) {
    ...
    try {
        ...
        if (!state.conversationId) {
            ...
            if (!state.runId && state.conversation?.intent?.ready_for_prompt)
                await startRun();
        }
    } catch (e) {
        toast(e.message);
    ...
    try {
        ...
        select.innerHTML =
            ...
            runs
                .map(
                    (r) =>
                        `<option value="${escapeHtml(r.run_id)}">${escapeHtml(r.run_id)} · ${escapeHtml(r.test_result || r.pipeline_status || "—")}</option>`,
                )
                .join("");
        stateEl.textContent = runs.length
            ? `${runs.length} saved job(s).`
            : "No saved jobs.";
    } catch (e) {
...
async function showJobDetail(runId) {
    ...
    if (!runId) {
        detail.innerHTML =
            '<p class="empty-note">Select a job to inspect its recorded state and workspace changes.</p>';
        return;
    }
    ...
    try {
        const [runRes, mutRes] = await Promise.all([
            ...
            apiFetch(
                `/api/runs/${encodeURIComponent(runId)}/artifacts/file-mutations`,
            ),
        ]);
        ...
        const rows = mutations?.records?.length
            ? mutations.records
                  .map(
                      (r) =>
                          `<tr><td><code>${escapeHtml(r.path)}</code></td><td>${escapeHtml(r.action)}</td><td>${r.bytes ?? "—"}</td><td><code>${r.sha256 ? r.sha256.slice(0, 16) + "…" : "—"}</code></td></tr>`,
                  )
                  .join("")
            : "";
...
function bindEvents() {
    ...
    $("#mode-close").addEventListener("click", () => {
        state.intentKind = "normal";
        updatePromptMode();
    });
    ...
    $("#explorer-toggle").addEventListener("click", () => {
        ...
        const showing =
            ...
            "var(--rail-w) 0px 1fr var(--task-w) var(--rail-w)";
        shell.style.gridTemplateColumns = showing
            ? "var(--rail-w) 0px 1fr var(--task-w) var(--rail-w)"
            : "";
    ...
    $("#flip-toggle").addEventListener("click", () => {
        ...
        $("#panel-title").textContent = isHistory
            ? "Job History"
            : "Current Job";
...
async function bootstrap() {
    ...
    if (state.runId) {
        ...
        try {
            ...
            toast(`Could not restore run: ${e.message}`);
        }
    }
