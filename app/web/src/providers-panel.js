...
export function createProviderPanel({ apiFetch, toast, onChanged } = {}) {
    ...
    async function refreshAndRender() {
        ...
        try {
            ...
            if (!data.providers.some((p) => p.id === selectedProvider))
                selectedProvider = data.providers.some(
            ...
            if (trigger)
                trigger.textContent = active
                    ...
                    : "Configure provider";
            root.innerHTML =
                `<p class="prov-summary">Active provider: ${esc(activeProvider === "<default>" ? "<default>" : data.providers.find((p) => p.id === activeProvider)?.displayName)}</p>` +
                ...
                data.providers
                    .map((p) => providerRowHTML(p, activeProvider))
                    .join("") +
            ...
            root.querySelectorAll("[data-provider]").forEach((row) => {
                ...
                row.querySelector("[data-key-visibility]")?.addEventListener(
                    "click",
                    (event) => {
                    ...
                    button.onclick = async () => {
                        ...
                        try {
                            if (action === "remove") {
                                ...
                                if (!value)
                                    throw new Error("Enter a replacement key.");
                                await request(base + "/credential", "PUT", {
                                    value,
                                });
                            } else {
                                const temperature =
            ...
            root.querySelectorAll("[data-select-provider]").forEach(
                (button) => {
                    button.onclick = () => {
                        clearSecrets();
                        ...
                        root.querySelectorAll("[data-provider]").forEach(
                            (row) => {
                                row.hidden =
                                    row.dataset.provider !== selectedProvider;
                            },
                        );
                    };
                },
    ...
    function clearSecrets() {
        document.querySelectorAll("#providers [data-key]").forEach((input) => {
            input.value = "";
            input.type = "password";
        });
        document
            .querySelectorAll("#providers [data-key-visibility]")
            .forEach((button) => {
