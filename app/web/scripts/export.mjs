        ...
        const rewritten = html.replace(
            ...
            (original, attrs, code) => {
                if (
                    /\bsrc\s*=/i.test(attrs) ||
                    !code.trim() ||
                    /\btype\s*=\s*["'](?!module|text\/javascript|application\/javascript)/i.test(
                        attrs,
                    )
                )
                    return original;
...
console.log(
    "Next.js static export published to app/web/dist; previous output retained under .runtime/frontend-builds.",
);
