...
function banner(text) {
    ...
    console.log(
        `${C.cyan}${C.bold}║${C.reset}  ${text.padEnd(58)}${C.cyan}${C.bold}║${C.reset}`,
    );
...
child.stderr.on("data", (data) => {
    ...
    if (errText.includes("EADDRINUSE")) {
        ...
        console.log(
            `To run OneShot on a different port, set the PORT environment variable:`,
        );
        console.log(
            `  ${C.cyan}Windows (PowerShell):${C.reset}  $env:PORT="8788"; npm run demo`,
        );
        console.log(
            `  ${C.cyan}Linux / macOS:${C.reset}        PORT=8788 npm run demo\n`,
        );
    }
});
...
child.stdout.on("data", async (data) => {
    ...
    if (text.includes("ONESHOT_SERVER_READY") && !isHandlingReady) {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(
                    `Health endpoint returned status ${res.status}`,
                );
            }
            ...
            log(
                `${C.dim}  2. Submit a request through the real Chat flow (or click example prompt)${C.reset}`,
            );
            log(
                `${C.dim}  3. Watch the canonical workflow execute live with real SSE events${C.reset}`,
            );
            log(
                `${C.dim}  4. Inspect the generated SHA-256 hash proof in the status bar & Proofs tab${C.reset}`,
            );
            ...
            try {
                ...
                log(
                    `${C.yellow}Could not open browser automatically. Open ${url} manually.${C.reset}`,
                );
            }
        } catch (err) {
            console.error(
                `${C.red}Backend health check failed: ${err.message}${C.reset}`,
            );
            child.kill("SIGTERM");
            process.exit(1);
        }
